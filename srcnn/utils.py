...
def fill_na(x, fillval=0):
    ...
    return tf.where(tf.math.is_finite(x), x, fill)

def nanmean(x, axis=None):
    x_filled = fill_na(x, 0)
    x_sum = tf.reduce_sum(x_filled, axis=axis)
    x_count = tf.reduce_sum(tf.cast(tf.math.is_finite(x), tf.float32), axis=axis)
    return tf.math.divide(x_sum, x_count)
...
def replicate_padding(tensor, pad_amt):
    if isinstance(pad_amt, (int, float)):
        pad_amt = [int(pad_amt)] * 2
